%RUN_FASHION_V3  Fashion-MNIST only. Stop HWiNFO logging when this finishes.
clear; clc;
...
diary(fullfile(OUTDIR, sprintf('console_Fashion_%s.txt', datestr(now,'yyyymmdd_HHMMSS')))); %#ok<TNOW1,DATST>
diary on
fprintf('Fashion-MNIST v3 started %s\n', datestr(now)); %#ok<TNOW1,DATST>
...
T_fashion = run_experiment_v3('Fashion-MNIST', XTr,YTr, XVa,YVa, XTe,YTe, ...
    ...
    hwinfoCsv = fullfile(HWDIR,'hwinfo_v3_fashion.csv'), ...
    idleWatts = IDLE_W, outDir = OUTDIR);

fprintf('Fashion-MNIST v3 finished %s\n', datestr(now)); %#ok<TNOW1,DATST>
diary off
